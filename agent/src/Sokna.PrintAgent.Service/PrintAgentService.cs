...
public sealed class PrintAgentService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        if (!_mutex.WaitOne(TimeSpan.Zero))
            throw new InvalidOperationException("نمونه دیگری از Sokna Print Agent فعال است.");
        ...
        try
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    // Operational priority is deliberate: preserve leases and completed evidence first,
                    // then print work, then acquire more work. Diagnostics must never starve printing.
                    ...
                    var state = _consecutiveApiFailures > 0 ? "degraded" : "running";
                    await WriteLocalHealthAsync(state, true, true, null, stoppingToken);
        ...
        finally
        {
            try
            {
                await WriteLocalHealthAsync("stopped", _api is not null, File.Exists(_paths.SecretPath), null, CancellationToken.None);
            }
            catch { }
    ...
    private async Task<bool> EnsureConfiguredAsync(CancellationToken ct)
    {
        ...
        if (!configExists || !secretExists)
        {
            _api = null;
            _destinations = [];
            var missing = !configExists && !secretExists ? "config و token" : !configExists ? "config" : "token";
            await WriteLocalHealthAsync("waiting_for_configuration", configExists, secretExists, $"در انتظار {missing}.", ct);
            return false;
        }

        var configStamp = File.GetLastWriteTimeUtc(_paths.ConfigPath);
        var secretStamp = File.GetLastWriteTimeUtc(_paths.SecretPath);
        if (_api is not null && configStamp == _configStampUtc && secretStamp == _secretStampUtc)
            return true;
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(token))
                throw new InvalidDataException("Agent token خالی است.");
            ...
            if (!probe.Success || probe.ProtocolVersion != 4)
                throw new InvalidOperationException("Print API v4 آماده نیست.");
        ...
        catch (Exception e)
        {
            ...
            _api = null;
            _destinations = [];
            if (e is not ApiOperationException)
                LogSafe("configuration", e);
    ...
    {
        if (_api is null || DateTimeOffset.UtcNow < _nextDestinationRefresh)
            return;
        ...
        {
            // Refresh is diagnostic/configuration freshness. Existing destination snapshots remain usable.
    ...
    {
        if (_api is null || _destinations.Count == 0)
            return;
    ...
    private async Task AcceptAnyReservedAsync(CancellationToken ct)
    {
        ...
        foreach (var local in (await _store.GetRecoverableAsync(ct))
                     .Where(x => x.State == LocalJobState.Reserved)
                     .OrderBy(x => x.ServerJobId)
                     .ThenBy(x => x.AttemptNo))
        {
            if (local.LeaseExpiresAt <= DateTimeOffset.UtcNow)
            {
                await _store.SetStateAsync(local.AttemptId, LocalJobState.Resolved, error: "Lease محلی پیش از Accept منقضی شد؛ Server Attempt جدید خواهد ساخت.", ct: ct);
                continue;
            }
            ...
            var item = new ClaimItem(
                ...
                new(local.AttemptId, local.AttemptNo, SecretStore.UnprotectText(local.ProtectedLeaseToken), ""),
                destination);
            var result = await RunApiAsync("accept", () => _api.AcceptAsync(item, local.LocalReceiptId, CryptoUtil.NewRequestId(), ct));
            if (result.Success)
    ...
    private async Task<bool> ProcessOneAsync(CancellationToken ct)
    {
        ...
        try
        {
            var started = await RunApiAsync("start", () => _api.StartAsync(job, CryptoUtil.NewRequestId(), ct));
            if (!started.Success)
                return false;
        }
        ...
        {
            TryKill(process);
            var durable = await TryReadWorkerResultAsync(job, CancellationToken.None);
            if (durable is not null)
                await ApplyWorkerResultAsync(job, durable, CancellationToken.None);
            else if (File.Exists(FencePath(job)))
                await QueueReportAsync(job, "recovery_hold", null, false, "worker_timeout_after_fence", "PrintWorker پس از Submission Fence Timeout شد؛ Retry خودکار ممنوع است.", CancellationToken.None);
            else
                await QueueReportAsync(job, "failed", null, true, "worker_timeout_before_fence", "PrintWorker پیش از Submission Fence Timeout شد؛ Retry ایمن مجاز است.", CancellationToken.None);
            CleanupWorkerFiles(job);
            return true;
        }
        catch (OperationCanceledException) when (ct.IsCancellationRequested)
        ...
        else
        {
            ...
            if (File.Exists(FencePath(job)))
                await QueueReportAsync(job, "recovery_hold", null, false, "worker_exit_without_result_after_fence", err.Length > 0 ? err : "Worker پس از Fence بدون نتیجه پایدار خاتمه یافت.", ct);
            else
                await QueueReportAsync(job, "failed", null, true, "worker_exit_without_fence", err.Length > 0 ? err : "Worker پیش از Fence بدون نتیجه پایدار خاتمه یافت.", ct);
        }
    ...
    private async Task ApplyWorkerResultAsync(LocalJob job, WorkerResult result, CancellationToken ct)
    {
        if (result.Status == "submitted")
        {
            ...
            {
                await QueueReportAsync(job, "recovery_hold", null, false, "submitted_without_spooler_id", "Worker وضعیت submitted بدون Spooler Job ID ثبت کرده است.", ct);
                return;
            }
            _lastSubmission = DateTimeOffset.UtcNow;
            await _store.SetStateAsync(job.AttemptId, LocalJobState.Submitted, result.SpoolerJobId, ct: ct);
            await QueueReportAsync(job, "submitted", result.SpoolerJobId, false, null, null, ct);
        }
        else if (result.Status == "failed")
        {
            await _store.SetStateAsync(job.AttemptId, LocalJobState.SafeFailed, error: result.ErrorMessage, ct: ct);
            await QueueReportAsync(job, "failed", null, result.Retryable, result.ErrorCode, result.ErrorMessage, ct);
        }
        else if (result.Status == "unknown")
        {
            await _store.SetStateAsync(job.AttemptId, LocalJobState.Unknown, result.SpoolerJobId, result.ErrorMessage, ct: ct);
            await QueueReportAsync(job, "unknown", result.SpoolerJobId, false, result.ErrorCode, result.ErrorMessage, ct);
        }
        else
        {
            await _store.SetStateAsync(job.AttemptId, LocalJobState.RecoveryHold, result.SpoolerJobId, result.ErrorMessage, ct: ct);
            await QueueReportAsync(job, "recovery_hold", result.SpoolerJobId, false, result.ErrorCode ?? "worker_recovery_hold", result.ErrorMessage, ct);
        }
    }
    ...
    {
        if (await _store.HasPendingReportAsync(job.AttemptId, ct))
            return;
        var requestId = CryptoUtil.NewRequestId();
        var body = JsonSerializer.Serialize(new { status, spooler_job_id = spooler, retryable, error_code = code, error_message = message }, AgentOptions.JsonOptions());
    ...
    private async Task FlushReportsAsync(CancellationToken ct)
    {
        ...
        foreach (var row in await _store.PendingReportsAsync(20, ct))
        {
            ...
            if (job is null)
            {
                await _store.MarkReportErrorAsync(row.Id, "Local attempt برای report پیدا نشد.", ct);
                continue;
            }
            ...
            try
            {
                ...
                var status = root.GetProperty("status").GetString() ?? "recovery_hold";
                ...
                var msg = root.TryGetProperty("error_message", out var em) && em.ValueKind == JsonValueKind.String ? em.GetString() : null;
                var result = await RunApiAsync("report", () => _api.ReportAsync(job, row.RequestId, status, spooler, retryable, code, msg, ct));
                if (result.Success)
                    await _store.MarkReportSentAsync(row.Id, row.AttemptId, ct);
                else
                    await _store.MarkReportErrorAsync(row.Id, Safe(result.Message ?? result.Code ?? "Server report rejected."), ct);
            }
            catch (ApiOperationException e)
            {
                await _store.MarkReportErrorAsync(row.Id, Safe(e.InnerException?.Message ?? e.Message), ct);
                // Outbox is durable. A network failure must not prevent already-claimed local work.
                break;
            }
            catch (Exception e)
            {
                await _store.MarkReportErrorAsync(row.Id, Safe(e.Message), ct);
            }
        }
    }
    ...
    {
        if (_api is null || DateTimeOffset.UtcNow < _nextHeartbeat)
            return;
        ...
        {
            // Heartbeat is observability, never an ownership/failover signal and never a print-path blocker.
    ...
    private async Task<T> RunApiAsync<T>(string action, Func<Task<T>> call)
    {
        ...
        try
        {
            ...
            _lastApiLatencyMs = sw.ElapsedMilliseconds;
    ...
    private static bool QueueReady(PrinterQueueHealth p, string queue) =>
        string.Equals(p.Name, queue, StringComparison.OrdinalIgnoreCase) && !p.Offline && !p.Paused && !p.PaperOut && !p.Error;
    ...
    {
        _log.LogError("{Area}: {Type}: {Message}", area, e.GetType().Name, Safe(e.Message));
        try { _fileLog.Error(area, e); }
