...
{
    private readonly HttpClient _http;private readonly string _baseUrl;private readonly ILeaseTokenProtector? _leaseProtector;
    ...
    public Task<ClaimResponse> ClaimAsync(string requestId,IReadOnlyCollection<string> readyDestinations,int limit,CancellationToken ct)=>PostAsync<ClaimResponse>("claim",new{request_id=requestId,agent_version=AgentVersionInfo.Current,protocol_version=4,limit,ready_destination_keys=readyDestinations},ct);
