...
public sealed class HttpPrintTransport: IPrintTransport
{
    ...
    private async Task<T> PostAsync<T>(string action,object body,CancellationToken ct)
    {
        ...
        var result=JsonSerializer.Deserialize<T>(text,AgentOptions.JsonOptions())??throw new InvalidDataException($"Print API {action} JSON نامعتبر است.");
        if(result is ApiResult api && !api.Success)throw new InvalidDataException($"Print API {action} business response ناموفق بود: {api.Code ?? api.Message ?? "unknown"}");
        return result;
    }
