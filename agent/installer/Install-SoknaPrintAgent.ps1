...
if(-not (Test-Path (Join-Path $source 'Service\Sokna.PrintAgent.Service.exe'))){throw 'Agent binaries are missing. Build the package first.'}
...
try{
  ...
  if(-not (Test-Path (Join-Path $stage 'Service\Sokna.PrintAgent.Service.exe'))){throw 'Staged Service executable is missing.'}
