...
sealed class TestEnvironment:IDisposable
{
    ...
    public PrintAgentService CreateService(IPrintTransport transport,IWorkerProcessFactory factory,LocalQueueStore? store=null,int workerTimeoutSeconds=20)
    {
        ...
        var service=new PrintAgentService(Paths,store,new ReadyPrinterHealthProvider(),NullLogger<PrintAgentService>.Instance,Log,new PrintWakeSignal(),dispatcher,new DurableMutationRequestStore(store),new WorkerSupervisor(factory));
